...
test.beforeEach(async ({ page }) => {
  ...
  await page.route('**/api/user', (route) => route.fulfill({ json: { name: 'Test Testsson', contract: 'Rörligt pris' } }))
...
test('fakturasidan visar det API:et svarar – även en faktura servern aldrig haft', async ({ page }) => {
  await page.route('**/api/invoices', (route) =>
    route.fulfill({ json: [{ id: 'F-999', period: 'December 2019', amount: 999, status: 'Obetald', due: '2020-01-01' }] })
  )
